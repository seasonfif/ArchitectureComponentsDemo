...
class RemoteDataSource : PageKeyedDataSource<String, Story>(){
    /**
     * Load initial data.
     *
     *
    ...
    override fun loadInitial(params: LoadInitialParams<String>, callback: LoadInitialCallback<String, Story>) {
        ...
        val response = ApiManager.create().getLatestNews().execute()
        if (response.isSuccessful){
            Log.e("loadInitial", response.body()!!.date)
            val list = response.body()!!.stories
            callback.onResult(list.toMutableList(), null, response.body()!!.date)
        }
    }
    ...
    override fun loadAfter(params: LoadParams<String>, callback: LoadCallback<String, Story>) {
        val response = ApiManager.create().getTheDaily(params.key).execute()
        if (response.isSuccessful){
            Log.e("loadAfter", response.body()!!.date)
            val list = response.body()!!.stories
            callback.onResult(list.toMutableList(), response.body()!!.date)
        }
    }
